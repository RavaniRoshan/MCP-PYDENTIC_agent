...
class GeminiVisionProvider(VisionLanguageProvider):
    ...
    async def process_request(
        ...
        try:
            ...
            # First, include the screenshot if available (as base64)
            contents = []
            ...
            # In a real scenario, we would include the screenshot of the browser
            # For now we'll just use the text prompt
            response = await model.generate_content_async(contents)
            
            # Parse the response
            if response and response.text:
                # Try to parse as JSON response (our expected format)
                try:
                    import re
                    ...
                    if json_match:
                        ...
                        # If no markdown, try the whole response
                        json_str = response.text.strip()
                    
                    # Clean up any non-JSON text around the response
                    if json_str.startswith('{') and json_str.endswith('}'):
                        ...
                        # If the response isn't JSON, return a simple response
                        return VisionLanguageResponse(
                            success=True,
                            actions=[{
                                "type": "navigate",
                                "value": "https://example.com",
                                "description": f"Navigate based on: {user_prompt.prompt}"
                            }],
                            reasoning=response.text[:200]  # First 200 chars as reasoning
                        )
                    ...
                    return VisionLanguageResponse(
                        ...
                        reasoning=response.text[:200] if response.text else "Generated by Gemini"
                    )
            else:
                return VisionLanguageResponse(
                    success=False,
    ...
    def _build_prompt(self, user_prompt: UserPrompt, browser_state: BrowserState) -> str:
        ...
        """
        return f"""You are an AI assistant that helps users perform tasks on websites by generating a sequence of browser actions. 
        The user has given the following request: '{user_prompt.prompt}'
        
        The current page information is:
        - URL: {browser_state.url}
        - Title: {browser_state.title}
        - Content: {browser_state.dom_content[:2000]}...  # Truncate to 2000 chars
